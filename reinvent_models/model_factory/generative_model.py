from reinvent_models.model_factory.configurations.model_configuration import (
    ModelConfiguration,
)
...
class GenerativeModel:
    def __new__(cls, configuration: ModelConfiguration) -> GenerativeModelBase:
        ...
        if cls._configuration.model_type == model_type_enum.DEFAULT:
            model = ReinventCoreAdapter(
                cls._configuration.model_file_path, mode=cls._configuration.model_mode
            )
        elif cls._configuration.model_type == model_type_enum.LIB_INVENT:
            model = LibInventAdapter(
                cls._configuration.model_file_path, mode=cls._configuration.model_mode
            )
        elif cls._configuration.model_type == model_type_enum.LINK_INVENT:
            model = LinkInventAdapter(
                cls._configuration.model_file_path, mode=cls._configuration.model_mode
            )
        else:
            raise ValueError(
                f"Invalid model_type provided: '{cls._configuration.model_type}"
            )
        return model
